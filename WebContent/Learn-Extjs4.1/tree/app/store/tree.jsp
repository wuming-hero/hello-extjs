...
<%	
  //要遍历树根和叶子节点,所以要定义两个ResultSet,但每个Statement只能对应一个ResultSet,所以要分别声明两个Statement
	...
	try{
		...
		//遍历查询根节点
		rs=sql.executeQuery("select * from tree where pid=0");		
		String root="";
		sql2 = con.createStatement();
		while(rs.next())
		{	//遍历查询叶子节点
			int id=rs.getInt(1);			
			rs2 = sql2.executeQuery("select * from tree where pid='" + id+ "'");
			String el="";	
	    	while (rs2.next()){ 
				el+="{id:"+rs2.getInt(1)+",text:'"+rs2.getString(2)+"',leaf:"+rs2.getString(3)+",checked:false},";
			}
	    	//得到每父节点所对应的全部叶子节点
		    el=el.substring(0,el.length()-1);
			root+="{id:"+rs.getInt(1)+",text:'"+rs.getString(2)+"',checked:false,children:["+el+"]},";
		} 
		//得到全部的json数组对象
