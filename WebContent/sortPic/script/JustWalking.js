var JSWK = {
	...
	opacity : function(d, c) {
		if (typeof c == "number") {
			...
			if (parseInt(c) >= 0) {
				var b = parseInt(c) * 10, a = parseInt(c) / 10;
				document.all
						? d.style.filter = "alpha(opacity=" + b + ")"
						: d.style.opacity = "" + a + ""
			} else {
	...
	lockScreen : function(f) {
		...
		if (objsbgDiv) {
			...
			if (navigator.userAgent.indexOf("MSIE 6.0") > 0) {
				...
				b.style.cssText = "position:absolute;width:"
						+ d
						+ "px;height:"
						+ c
			...
			a.style.cssText = "position:absolute;top:0px;left:0px;background:#333333;width:"
					+ d + "px;height:" + c + "px;z-index:10;";
			JSWK.opacity(a, f);
	...
	createDivIframe : function(g, k, e, f, c) {
		...
		if (Jid("JSWK_CreateDiv")) {
			...
			m.innerHTML = "";
			if (e == "true") {
				m.innerHTML += "<div id='theMoveArea'  moveArea='true' style='cursor:move;' class='"
						+ c + "'>" + f + "</div>"
			} else {
				m.innerHTML += "<div id='theMoveArea'  moveArea='false' style='cursor:move;display:none;'></div>"
			}
			m.innerHTML += "<iframe id='JSWK_CreateIframe' application='yes' frameborder='0' scrolling='no' marginheight='0' marginwidth='0' name='JSWK_CreateIframe' width='"
					+ l
					+ "' height='"
					+ b
	...
	setNewH : function(a) {
		...
		var d = a.width;
		var c = a.height;
		Jid("JSWK_CreateDiv").style.left = (b / 2 - parseInt(d) / 2)
				+ parseInt(g) + "px";
		Jid("JSWK_CreateDiv").style.top = (f / 2 - parseInt(c) / 2)
				+ parseInt(e) + "px"
	},
	_h : "http://",
	toCenter : function(a) {
	...
	autoToSize : function(a, j, k) {
		...
		var d = g.getChilds();
		if (d.length < 1) {
			g.appChild("<p style='width:" + g.clientWidth + "px; height:"
					+ g.clientHeight + "px; visibility:hidden;'>&nbsp;</p>")
		}
		d = g.getFirst();
		var b = sH = 0;
		b = (j - g.clientWidth) / f;
	...
	clewBox : function(e, d, c) {
		if (Jid("JSWK_clewBoxDiv")) {
			...
			b += "<span class='" + d + "'></span>" + e
					+ "<span class='clew_end'></span>";
			b += "</span></div>";
		...
		if (document.getElementById) {
			(function() {
				...
				function g(n) {
					...
					var d = (typeof n.target != "undefined")
							? n.target
							: n.srcElement;
					if (d.getAttribute("moveArea") == "true") {
						d = (typeof d.parentNode != "undefined")
								? d.parentNode
								: d.parentElement;
						k = true;
						d.style.zIndex = c++;
	...
	initframeHeight : function(e) {
		if (e.contentDocument) {
			var a = Math.max(e.contentDocument.body.offsetWidth,
					e.contentDocument.documentElement.offsetWidth,
					e.contentDocument.body.scrollWidth,
					e.contentDocument.documentElement.scrollWidth);
			e.width = a + "px";
			e.parentNode.style.width = a + "px";
			var d = Math.max(e.contentDocument.body.offsetHeight,
					e.contentDocument.documentElement.offsetHeight,
					e.contentDocument.body.scrollHeight,
					e.contentDocument.documentElement.scrollHeight);
			e.height = d + "px";
			e.parentNode.style.height = d + "px"
		} else {
			if (e.Document) {
				var c = Math.max(e.Document.body.offsetWidth,
						e.Document.documentElement.offsetWidth,
						e.Document.body.scrollWidth,
						e.Document.documentElement.scrollWidth);
				e.width = c + "px";
				e.parentNode.style.width = c + "px";
				var b = Math.max(e.Document.body.offsetHeight,
...
function Jtag(c) {
	if (typeof c == "string") {
		var a = [], b = document.getElementsByTagName(c);
...
function Jids(d, a) {
	if (typeof d == "string") {
		var b = [], c = a ? Jid(a).getElementsByTagName("*") : document
